...
cp init-usb_gadget /usr/bin/ # USB gadget configFS
chmod +x /usr/bin/init-usb_gadget
sed -i '/^exit 0/i /usr/bin/init-usb_gadget' /etc/rc.local # libcomposite configuration
